...
def colorize(img_path, model):
    ...
    gray = dataset.HandleGrayscale()
    img = gray(img)
    
    crop = dataset.RandomCrop(224)
    img,h,w = crop(img,resize=True)
    ...
    net.eval()
    with torch.no_grad():
        L_out,ab_out, predicted = net(L_tensor)
        img_colorized = utils.net_out2rgb(L_out[0],ab_out[0])
        print(img_colorized.shape)
        #io.imread(img_colorized,format="TIFF")
        #print(img_colorized)
        
        colorized_img_name = "colorizedLIR-" + img_name
