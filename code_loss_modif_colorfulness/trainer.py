...
class Training:
    ...
    def colorfulness_loss(self, col_target, col_out):
        # Just the difference between the mean and standard deviation of a*b*
        mean_target = torch.mean(col_target)
        std_target = torch.var(col_target)

        mean_out = torch.mean(col_target)
        std_out = torch.var(col_target)

        return torch.abs(mean_target + std_target - 0.37 * (mean_target + std_target))
    ...
    def loss(self, col_target, col_out, class_target, class_out):
        ...
        loss_colorfulness = self.colorfulness_loss(col_target, col_out)
        return loss_col*(1+5*loss_colorfulness) + loss_class/300.0
